...
function Trending() {
    ...
    const topics = data?.categoriesCollection?.edges || []
    ...
    return (
        <div className="mt-4 ml-10 my-7 bg-base-100 p-5 border rounded-t-2xl rounded-b-2xl shadow-sm">
            ...
            {
                topics.map(topic => (
                    <div key={topic.node.id} className="flex items-center justify-between mt-3">
                        <img className="w-10 h-10 rounded-full border p-[2px]" src={faker.image.image()} />
                        <Link href={`/category/${topic.node.name}`}>
                            <div className="flex-1 ml-4 hover:text-info hover:underline cursor-pointer">
                                <h2 className="font-semibold text-sm">p/{topic.node.name}</h2>
                            </div>
                        </Link>
                    </div>
                ))
