...
import { SEARCH_USERS_BY_USERNAME } from '../graphql/queries';
...
export default function SearchBar({ placeholder }) {
    ...
    async function fetchUsers(word) {
        const { data: { searchUsersByUsername } } = await client.query({
            query: SEARCH_USERS_BY_USERNAME,
            variables: {
                username: word + '%'
            }
        })
        setFilteredData(searchUsersByUsername)
    }
        ...
        <div className="bg-base-100 rounded-md">
            <div className="inline-flex flex-col flex-start justify-center relative text-inherit">
                <div className="flex items-center">
                    <input type="text" className="p-2 pl-8 h-8 rounded-md bg-base-100 text-inherit focus:ring-primary focus:border-primary"
                        value={wordEntered}
                    ...
                    {filteredData.length === 0 ?
                        ...
                        <button onClick={clearInput}>
                            <svg className="w-4 h-4 absolute left-2.5 top-2.5 cursor-pointer" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
