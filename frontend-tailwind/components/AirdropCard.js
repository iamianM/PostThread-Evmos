...
export default function AirdropCard() {
    ...
    const resetAirdrop = async (event) => {
        // Stop the form from submitting and refreshing the page.
        event.preventDefault()
        try {
            await client.mutate({
            ...
            toast.success("Airdrop successfully reset!")
        } catch (error) {
            toast.error("Something went wrong, please try again!")
        }
    }

    let airdropComponent = null

    if ((data?.getUsers?.reddit_username)) {
        switch (data?.getUsers?.reddit_airdrop_claimed) {
            case "not_claimed":
                airdropComponent =
                    data?.getUsers?.reddit_airdrop_value ?
                        <>
                        ...
                        <>
                            <div className="flex justify-center mb-10 items-center space-x-2 m-4">
                                <p className='font-bold text-xl'> We have received your reddit info, come back soon to claim your Airdrop</p>
                            </div>
                        </>
                break;
            case "rejected":
                airdropComponent =
                    <>
                        <div className="flex justify-center items-center space-x-2 m-4">
                            <p className='font-bold text-xl'> Something went wrong with your Airdrop, please press the button below to reset your request</p>
                        </div>
                        <button onClick={(e) => resetAirdrop(e)}
                            className="bg-primary px-4 py-2 mb-4 rounded-xl font-semibold text-inherit hover:bg-secondary-focus focus:ring focus:ring-purple-500 focus:ring-opacity-25 outline-none"
                        >
                            Reset
                        </button>
